...
describe('OpenQuota dashboard', () => {
  ...
  it('uses the native system language for settings and new appearance controls', async () => {
    const invoke = mocks.invoke.getMockImplementation()!;
    mocks.invoke.mockImplementation(async (command: string, args?: InvokeArgs) => {
      ...
      return {
        ...
        settings: {
          ...
          settings: { ...result.settings.settings, language: 'system' },
        },
      };
    });
    render(App);
    await screen.findByRole('combobox', { name: '總消費指標' });
    await fireEvent.click(screen.getByLabelText('開啟選項'));
    await fireEvent.click(screen.getByRole('button', { name: '設定' }));
    expect(screen.getByRole('combobox', { name: '語言' })).toHaveTextContent('系統');
